...
TEST(ShortestSuperseq, CustomExample1) {
  ...
  std::vector<int> in2 = {0, 5, 9, 11, 7, 2, 1, 1, 4,
                          3, 8, 8, 23, 7, 3, 3, 7, 9};
...
TEST(ShortestSuperseq, CustomExample2) {
  std::vector<int> in1 = {2, 3, 5, 13};
  std::vector<int> in2 = {2, 4, 3,  9, 7, 8, 1, 1, 4, 3,
                          8, 8, 23, 5, 7, 3, 3, 7, 9, 13};

  SubArray test_result = ShortestSuperseq(in1, in2);

  EXPECT_EQ(test_result.start, 0);
  EXPECT_EQ(test_result.end, 19);
}